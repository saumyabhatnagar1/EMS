...
export class ViewTasksComponent implements OnInit {
  ...
  updateTask() {
    ...
    this.viewTaskService.updateStatus(JSON.stringify(data)).subscribe(
      res => {
        ...
        console.log(res)
        this.getTask()
        this.messageService.add({ severity: 'success', summary: 'Task updated...', life: 2000 })
      },
      err => {
        console.log(err)
      }
  ...
  getTeam() {
    ...
    this.viewTaskService.getTeam(JSON.stringify(data)).subscribe(
      res => {
        let res1 = Object.entries(res)
        console.log(res1)
        ...
        for (let i = 0; i < res1.length; i++) {
          this.projectTeam.push(
            { label: res1[i][1].emp_id, value: res1[i][1].emp_id }
          )
        }
